...
export const Route = createFileRoute("/alerts")({
  ...
  head: () => ({
    meta: [{ title: "Alerty cenowe · Voya" }, { name: "description", content: "Śledź ceny lotów i hoteli. Otrzymuj powiadomienia gdy cena spadnie poniżej progu." }],
  }),
});
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 font-display text-3xl font-bold sm:text-4xl">Twoje alerty cenowe</h1>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {alerts.map((a) => <AlertCard key={a.id} a={a} onToggle={() => toggle(a.id)} />)}
        </div>
      </div>
...
function AlertCard({ a, onToggle }: { a: PriceAlert; onToggle: () => void }) {
  ...
  return (
    <div className={`rounded-3xl border p-5 transition-opacity ${a.active ? "border-border bg-card" : "border-border bg-muted/50 opacity-70"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-muted text-2xl">{a.flag}</span>
          <div>
            <div className="font-display text-lg font-semibold">{a.route}</div>
            <div className="mt-0.5 flex flex-wrap gap-1">
              {a.vibes.map((v, i) => <span key={i} className="rounded-full bg-muted px-2 py-0.5 text-xs">{v}</span>)}
            </div>
          </div>
        </div>
      ...
      <div className="mt-5 flex items-end justify-between gap-4">
        <div>
          ...
          <div className={`font-display text-3xl font-bold ${below ? "text-brand-green" : "text-foreground"}`}>
            {a.current} <span className="text-base font-medium text-muted-foreground">{a.currency}</span>
          </div>
          <div className={`mt-1 inline-flex items-center gap-1 text-xs font-medium ${dropped ? "text-brand-green" : "text-brand-pink"}`}>
            {dropped ? <TrendingDown className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}
            {a.change > 0 ? "+" : ""}{a.change}% / 30 dni
          </div>
        </div>
        <Sparkline data={a.history} threshold={a.threshold} min={Math.min(...a.history) * 0.95} max={Math.max(...a.history) * 1.05} />
      </div>
      ...
      <div className="mt-4 flex items-center justify-between rounded-2xl bg-muted/60 p-3 text-xs">
        <div>
          ...
          <div className="font-semibold">{a.threshold} {a.currency}</div>
        </div>
...
function Sparkline({ data, threshold, min, max }: { data: number[]; threshold: number; min: number; max: number }) {
  const W = 160, H = 60;
  ...
  return (
    <svg width={W} height={H} className="shrink-0">
      ...
      <polyline points={points} fill="none" stroke="oklch(0.62 0.20 245)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <line x1="0" x2={W} y1={ty} y2={ty} stroke="oklch(0.70 0.18 155)" strokeDasharray="3 3" strokeWidth="1.5" />
      <circle cx={lastX} cy={lastY} r="3.5" fill="oklch(0.62 0.20 245)" stroke="white" strokeWidth="2" />
    </svg>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-border bg-card p-6 shadow-pop">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-brand-yellow text-xl text-brand-yellow-ink">🔔</span>
            <div>
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            <span>Próg ceny</span><span className="text-foreground">{threshold} zł</span>
          </div>
          <input type="range" min={500} max={4000} step={50} value={threshold} onChange={(e) => setThreshold(+e.target.value)} className="w-full accent-[oklch(0.62_0.20_245)]" />
          <div className="mt-1 flex justify-between text-[10px] text-muted-foreground"><span>500 zł</span><span>4000 zł</span></div>
        </div>

        <div className="mt-5">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Powiadamiaj przez</div>
          <div className="grid grid-cols-2 gap-2">
            {(["email", "push"] as const).map((k) => (
              <button key={k} onClick={() => setCh((c) => ({ ...c, [k]: !c[k] }))}
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-full border border-border px-4 py-2 text-sm">Anuluj</button>
          <button onClick={onClose} className="rounded-full bg-brand-blue px-5 py-2 text-sm font-semibold text-white">Utwórz alert</button>
        </div>
      </div>
    </div>
  );
